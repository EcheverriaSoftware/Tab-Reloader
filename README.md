...
See [docs/](docs/) for the documentation hub, or jump to a spec: the
[IT Workflow PRD](docs/PRD.md) (the three pillars), the
[events spec](docs/EVENTS_PRD.md), or the
[DMP analysis spec](docs/PRD-DMP-Analysis.md).
...
- **Bugcheck decoding**: the stop code is decoded to its symbolic name (e.g.
  ...
  base for the common codes and a name for ~230 published codes.
...
src/
  ...
  analyzer/              DMP File Analyzer (its own page)
    ...
    module-vendors.js    module filename → vendor / product / hardware category lookup
